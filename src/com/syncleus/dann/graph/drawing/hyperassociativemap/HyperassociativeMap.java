...
//          [needs-be-pos-1 needs-be-pos-2]))
	...
	{
		return totalMovement / graph.getNodes().size();
	}
	...
	public void align()
	{
		...
		else
		{
			...
			{
				warn("waitAndProcessFutures was unexpectedly interrupted", caught);
				throw new UnexpectedInterruptedException("Unexpected interruption. Get should block indefinitely", caught);
			}
		}

		debug("maxMove: " + maxMovement + ", Average Move: " + getAverageMovement());
	...
	private Vector processLocally()
	{
		...
		if ((learningRate * LEARNING_RATE_PROCESSING_ADJUSTMENT) < DEFAULT_LEARNING_RATE)
		{
			...
			debug("learning rate: " + learningRate + ", acceptableDistanceFactor: " + acceptableDistanceFactor);
		}
		return pointSum;
	}
	...
	private Vector waitAndProcessFutures(final List<Future<Vector>> futures) throws InterruptedException
	{
		...
		{
			error("Align had an unexpected problem executing.", caught);
